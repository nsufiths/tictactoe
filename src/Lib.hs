...
getCoords (GameState f OPlayer) =  if f !! 1 !! 0 == Empty then (0,1) else if  f !! 1 !! 1 == Empty then (1,1) else (2,1)
...
gamePlay = do 
    ...
    if winner /= None then return winner else do
        ...
        putMark coords
        let newPlayer = if currentPlayer st == XPlayer then OPlayer else XPlayer
        st <- get
        put (st {currentPlayer = newPlayer})
        gamePlay
